class SortObjects {
    ...
    static <T> void sort(T[] objects, java.util.Comparator<T> comparator) {          // void hat gefehlt
        for(int currentIndex = 0; currentIndex < objects.length; currentIndex++) {
            T currentObject = objects[currentIndex];                                 // Datentyp war falsch
            int insertionPosition = currentIndex;
            while(insertionPosition > 0 && comparator.compare(objects[insertionPosition - 1], currentObject) > 0) { // Methodenname war falsch
                objects[insertionPosition] = objects[insertionPosition - 1];
                insertionPosition--;
            }
            objects[insertionPosition] = currentObject; 
    ...
    static void main() {
        ...
        sort(studis, nachName); // Sortierung nach Name
