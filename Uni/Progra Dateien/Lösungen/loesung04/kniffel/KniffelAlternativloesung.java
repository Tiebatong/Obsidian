// andere Lösungsansätze für den unteren Block (Dreierpasch, Viererpasch, Kleine Straße, Große Straße, Kniffel)
...
static int smallStraight(int[] dice) {
    // Idee: Eine kleine Straße liegt genau dann vor, wenn die Augenzahlen 1-4 (Variante 1), 2-5 (Variante 2) oder 3-6 (Variante 3) mindestens einmal vorkommen.

    int[] frequencies = countFrequencies(dice);
    
    boolean variante1 = frequencies[0] >= 1 && frequencies[1] >= 1 && frequencies[2] >= 1 && frequencies[3] >= 1;
    boolean variante2 = frequencies[1] >= 1 && frequencies[2] >= 1 && frequencies[3] >= 1 && frequencies[4] >= 1;
    boolean variante3 = frequencies[2] >= 1 && frequencies[3] >= 1 && frequencies[4] >= 1 && frequencies[5] >= 1;
    
    if(variante1 || variante2 || variante3) {
        return 30;
    } else {
        return 0;
    }
...
static int largeStraight(int[] dice) {
    // Idee: Wenn die Augenzahlen sortiert sind, liegt eine Große Straße vor,
    // wenn es genau vier direkt aufeinanderfolgende Augenzahlen gibt.
...
static int kniffel(int[] dice) {
    // Idee: Alle Würfel müssen dieselbe Augenzahl zeigen wie der erste Würfel.
    for(int die: dice) {
        if(die != dice[0]) {
            return 0;
        }
